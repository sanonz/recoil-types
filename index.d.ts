...
export interface ReadOnlySelectorOptions<T, R = T> {
   key: string;
   get: (opts: { get: GetRecoilValue }) => Promise<T> | RecoilValue<T> | T;
   dangerouslyAllowMutability?: boolean;
}

export interface ReadWriteSelectorOptions<T, R = T> extends ReadOnlySelectorOptions<T> {
 set: (
   opts: {
     set: SetRecoilState;
     get: GetRecoilValue;
   ...
   newValue: R | DefaultValue,
 ) => void;
}

export function selector<T, R = T>(options: ReadWriteSelectorOptions<T, R>): RecoilState<T, R>;
export function selector<T, R = T>(options: ReadOnlySelectorOptions<T, R>): RecoilValueReadOnly<T, R>;
...
// hooks.d.ts
export type SetterOrUpdater<T, R = T> = (valOrUpdater: ((currVal: T) => R) | R) => void;
...
*/
export function useRecoilState<T, R = T>(recoilState: RecoilState<T, R>): [T, SetterOrUpdater<T, R>];
...
*/
export function useRecoilStateLoadable<T, R = T>(recoilState: RecoilState<T, R>): [Loadable<T>, SetterOrUpdater<T, R>];
...
export function useSetRecoilState<T, R = T>(recoilState: RecoilState<T, R>): SetterOrUpdater<T, R>;
...
export class RecoilState<T, R = T> extends AbstractRecoilValue<T> {}
export class RecoilValueReadOnly<T, R = T> extends AbstractRecoilValueReadonly<T> {}
...
export interface ReadWriteSelectorFamilyOptions<T, P extends SerializableParam, R = T> {
 key: string;
 get: (param: P) => (opts: { get: GetRecoilValue }) => Promise<T> | RecoilValue<T> | T;
 set: (
     param: P,
...
export function selectorFamily<T, P extends SerializableParam, R = T>(
 options: ReadWriteSelectorFamilyOptions<T, P, R>,
): (param: P) => RecoilState<T, R>;
